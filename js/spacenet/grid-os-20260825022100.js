...
function grokOnly() {
  ...
  form.addEventListener("submit", function (e) {
    ...
    fetch("/api/ai", {
      ...
      body: JSON.stringify({ message: v, gift: true, allow_paid: true, force_paid: true }),
    }).then(function (r) { return r.json(); }).then(function (j) {
      if (lineEl) { lineEl.textContent = String(j.text || ""); lineEl.classList.remove("gone"); }
    }).catch(function () {});
  });
}
if (!window.THREE || !canvas) { grokOnly(); return; }
...
async function geocode(q) {
  try {
    const r = await fetch("https://photon.komoot.io/api/?q=" + encodeURIComponent(q) + "&limit=1", { signal: to(6000) });
    if (!r.ok) return null;
...
function locate() {
  return new Promise((resolve) => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { matter(meBtn, true); resolve(null); },
...
callBtn.onclick = () => { const v = sel(); hailVendor(v, !!(v && v.phone)); };
